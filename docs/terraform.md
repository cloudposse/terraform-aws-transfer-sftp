...
| <a name="input_sftp_users"></a> [sftp\_users](#input\_sftp\_users) | List of SFTP usernames and public keys. The keys `user_name` and `public_keys` are required. The key `s3_bucket_name` is optional. | <pre>list(<br/>    object({<br/>      user_name      = string<br/>      public_keys    = list(string)<br/>      s3_bucket_name = optional(string)<br/>    })<br/>  )</pre> | `[]` | no |
