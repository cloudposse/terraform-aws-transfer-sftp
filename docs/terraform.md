...
| <a name="input_home_directory"></a> [home\_directory](#input\_home\_directory) | The home directory if `restricted_home` is `true`. The default uses the format `/{var.s3_bucket_name}`. | `string` | `null` | no |
| <a name="input_home_directory_mappings"></a> [home\_directory\_mappings](#input\_home\_directory\_mappings) | The home directory mappings if `restricted_home` is `true`. The default value can be overridden by using the key `default`. | <pre>map(object({<br>    entry  = string,<br>    target = string<br>  }))</pre> | `{}` | no |
